...
edges = cv.Canny(adaptive_thresh, 50, 150)
...
# 특징점 필터링(반응값 기준)
#filtered_keypoints = [kp for kp in keypoints if kp.response > 0.01] # 너무 작은 특징 제거 
...
# 결과 출력 
while True:
    cv.imshow("ORB Keypoints", output_image)
    cv.imshow("ORB Check", edges)
